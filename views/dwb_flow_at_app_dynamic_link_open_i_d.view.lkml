view: dwb_flow_at_app_dynamic_link_open_i_d {
  sql_table_name: `dwb_utc_00.dwb_flow_at_app_dynamic_link_open_i_d`
    ;;
  ...
  dimension_group: user_first_touch_timestamp {
    type: time
    description: "用户首次交互时间戳，如在 app 场景下，用户首次打开 app 时的时间戳"
    timeframes: [
      raw,
      time,
      date,
...
view: dwb_flow_at_app_dynamic_link_open_i_d__event_params {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: dwb_flow_at_app_dynamic_link_open_i_d__user_properties {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
